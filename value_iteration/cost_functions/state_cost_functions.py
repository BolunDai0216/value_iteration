...
class FeatureQuadraticCost:
    def __init__(self, Q, feature=None, cuda=False):
        ...
        self.to_notf = torch.from_numpy(
            self.np_notf).float().view(1, self.n, 1)
    ...
    def grad_numpy(self, x):
        ...
        x = x.reshape((-1, self.n, 1))
        f = self.np_notf * x + self.np_f * self.feature_numpy(x)
        dfdx = (self.np_notf * np.ones_like(x) + self.np_f *
                self.grad_feature_numpy(x)).reshape((-1, self.n, 1))
        return 2. * np.matmul(self.np_Q, f).reshape((-1, self.n, 1)) * dfdx

    def grad_torch(self, x):
        x = x.reshape((-1, self.n, 1))
        f = self.to_notf * x + self.to_f * self.feature_torch(x)
        dfdx = (self.to_notf * torch.ones_like(x) + self.to_f *
                self.grad_feature_torch(x)).view(-1, self.n, 1)
    ...
    def hessian_torch(self, x):
        ...
        dfdx = (self.to_notf * torch.ones_like(x) + self.to_f *
                self.grad_feature_torch(x)).view(-1, self.n, 1)
        d2fd2x = torch.diag_embed(
            (self.to_notf * torch.zeros_like(x) + self.to_f * self.hessian_feature_torch(x)).squeeze(-1))
...
if __name__ == "__main__":
    ...
    x0 = np.hstack([np.linspace(-np.pi, np.pi, n, endpoint=True)
                   [:, np.newaxis], np.zeros(n)[:, np.newaxis]])
    ...
    torch_c_quad, torch_dcdx_quad = fc_quad(torch_x0).detach(
    ).numpy(), fc_quad.grad(torch_x0).detach().numpy()
    torch_c_feat, torch_dcdx_feat = fc_feat(torch_x0).detach(
    ).numpy(), fc_feat.grad(torch_x0).detach().numpy()
    torch_c_sine, torch_dcdx_sine = fc_sine(torch_x0).detach(
    ).numpy(), fc_sine.grad(torch_x0).detach().numpy()
    torch_c_cosi, torch_dcdx_cosi = fc_cosi(torch_x0).detach(
    ).numpy(), fc_cosi.grad(torch_x0).detach().numpy()

    assert np.allclose(torch_c_quad, c_quad) and np.allclose(
        torch_dcdx_quad, dcdx_quad)
    assert np.allclose(torch_c_feat, c_feat) and np.allclose(
        torch_dcdx_feat, dcdx_feat)
    assert np.allclose(torch_c_sine, c_sine) and np.allclose(
        torch_dcdx_sine, dcdx_sine)
    assert np.allclose(torch_c_cosi, c_cosi) and np.allclose(
        torch_dcdx_cosi, dcdx_cosi)
    ...
    fig = plt.figure()
    ax = [fig.add_subplot(1, feature.size+1, i+1)
