...
def run_experiment(hyper):
    ...
    if hyper["plot"]:
        ...
        x_lim = scale * \
            torch.tensor([system.x_lim[0], system.x_lim[1]]).float()
        ...
        fig.subplots_adjust(left=0.05, bottom=0.12, right=1.0,
                            top=0.93, wspace=0.1, hspace=0.3)

        def format_space_ax(ax):
            y_ticks = [-7.5, 0.0, +7.5]
            x_ticks = [-np.pi / 1., -np.pi / 2., 0.0, np.pi / 2., np.pi]
            x_tick_label = [r"$\pm\pi$", r"$-\pi/2$",
                            r"$0$", r"$+\pi/2$", r"$\pm\pi$"]
        ...
        plot_hyper = {'levels': 50, 'norm': norm_V,
                      'cmap': cm.get_cmap(cm.Spectral, 50)}
        ...
        plot_hyper = {'levels': 50, 'norm': norm_u,
                      'cmap': cm.get_cmap(cm.Spectral, 50)}
        ...
        fig.subplots_adjust(left=0.065, bottom=0.1, right=0.98,
                            top=0.97, wspace=0.1, hspace=0.3)
        ...
        for i in range(n_plot):
            xi_tra = add_nan(np.concatenate(
                (x_tra[:, i, :, 0], t[:, np.newaxis]), axis=-1), system.wrap_i)

            ax_xp.plot(xi_tra[:, -1], xi_tra[:, 0], c="b", alpha=0.25)
